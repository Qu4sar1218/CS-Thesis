...
def record_attendance(student_id, name, course, year, first_name="", middle_name="", last_name=""):
    ...
    with attendance_lock:
        ...
        if not today_records:
            ...
            attendance_queue.put(record)  # Queue for async save
            ...
            try:
                ...
                if current_mode == "class" and current_class_id:
                    loop.run_until_complete(
                        update_attendance_status(record['student_id'], current_class_id, 'present')
                    )
            finally:
                loop.close()
        except queue.Empty:
...
PROCESS_EVERY_N_FRAMES = 15  # Increased from 10 to reduce recognition frequency
JPEG_QUALITY = 50  # Reduced from 70 for faster JPEG encoding
FRAME_SCALE = 0.25  # Reduced from 0.5 for faster processing
...
def recognition_loop():
    """Optimized recognition loop with minimal lag."""
    ...
    while recognition_running and not stop_streaming:
        ...
        # ONLY detect faces every N frames, not every frame!
        if HAVE_FACE_RECOG and frame_count % PROCESS_EVERY_N_FRAMES == 0:
            small_frame = cv2.resize(frame, (0, 0), fx=FRAME_SCALE, fy=FRAME_SCALE)
            rgb_small = cv2.cvtColor(small_frame, cv2.COLOR_BGR2RGB)

            # Detect faces (only now, not every frame!)
                ...
                for face_idx, ((top, right, bottom, left), enc) in enumerate(zip(faces, encs)):
                    ...
                    if len(dists) > 0:
                        ...
                        if dists[idx] <= FACE_MATCH_THRESHOLD:
                            ...
                            first_name = known_face_first_names[idx]
                            middle_name = known_face_middle_names[idx]
                            last_name = known_face_last_names[idx]

                            # Non-blocking attendance recording
                            record_attendance(student_id, name, course, year,
                                            first_name, middle_name, last_name)

                    last_names[face_idx] = name
